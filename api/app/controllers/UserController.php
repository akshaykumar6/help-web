...
class UserController extends BaseController {
	...
	public static function login() {
		$payloadArray = self::getAllParam();
		$userDetails = User::where('ngo_user_username',$payloadArray['ngoUserName'])->get()->toArray();
		
		$response = array();
		if ($userDetails) {
			if( $userDetails[0]['ngo_user_password'] == $payloadArray['password']) {
				$response['success'] = 1;
				$response['ngoUserID'] = $userDetails[0]['ngo_user_id'];
